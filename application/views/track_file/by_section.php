                        ...
                        <table class="table">
                          ...
                          <?php if(isset($results)){foreach($results as $key=>$value){?>
                            <tr>
                             ...
                             <td><?php   if($results[$key]==true){ ?><a href="<?php echo base_url().'file_inbox/file_view/?fid='.id_encrypt($value["file_id"]);?>" style="color:#5fc4c8"><?php } else{  ?> <a class=" waves-effect waves-light" data-toggle="modal" data-target="#panel-modal" style="color:#5fc4c8;cursor:pointer"><?php } echo $value["file_ref_sl_no"];?></a></td>
                              <td style="width:50%;"><?php echo $value["subject"];?></td>
                              <td><?php if($value["reciver_user_id"]!=0) echo $value["name"]; 
							 else echo(fetch_user_name($value["reciver_user_id"]));?></td>
                              <td><?php echo wordwrap($value["sec_name"],25,"<br>\n");?></td>
                              <td><?php 
