<div class="row">
          ...
          <div class="panel-heading"><svg class="glyph stroked email"></svg>Track By File Id</div>
          <div class="panel-body">
            <div class="col-md-12">
          
          <?php echo form_open('file_inbox/track_file_id',"class='form-horizontal'");?>  
 					<fieldset>
							
  <div class="form-group">
  <label for="name" class="col-md-3 control-label">Search<span style="color:red; font-size:20px">*</span>:</label>
    <div class="col-md-3 has-success">
    <input type="text" class="form-control" id="id" name="id" value=<?php  echo $this->session->userdata('file_id'); ?> >
                          ...
                          <?php if(isset($results)){foreach($results as $key=>$value){?>
                            <tr>
                                ...
                                <td class="text-center">
									...
									<?php  if($value["permission"]==true){ ?><a class=" btn btn-default btn-sm" style="background:#1ebfae;color:#ffffff;border-radius:5px;" href="<?php echo base_url().'file_inbox/file_view/?fid='.id_encrypt($value["file_id"]);?>" style="color:#5fc4c8"><?php } else{  ?> <a class=" btn btn-default btn-sm" data-toggle="modal" data-target="#panel-modal" style="background:#1ebfae;color:#ffffff;border-radius:5px;"><?php } echo $value["file_id"];?></a>
									
									
								
								<?php if($value["file_type"]=="CRIME") { ?>
									
									
									<?php  if($value["permission"]==true){ ?><a class="btn btn-danger btn-circle" style="background:#f39c12;border:none;" href="<?php echo base_url().'attach_to_file/view_plan/'.$value["file_id"];?>" style="color:#5fc4c8"> POI</a>
                              ...
                              <td><?php  if($value["permission"]==true){ ?><a href="<?php echo base_url().'file_inbox/file_view/?fid='.id_encrypt($value["file_id"]);?>" style="color:#5fc4c8"><?php } else{  ?> <a class=" waves-effect waves-light" data-toggle="modal" data-target="#panel-modal" style="color:#5fc4c8;cursor:pointer"><?php } echo $value["file_ref_sl_no"];?></a></td>
